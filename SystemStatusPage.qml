...
Rectangle {
    ...
    ColumnLayout {
        ...
        Label {
            text: qsTr("设备连接: 未连接（模拟数据源）")
            color: "#d9e4ec"
            font.pixelSize: 16
        }

        Label {
            text: qsTr("采集状态: ") + (root.simulationRunning ? qsTr("运行中") : qsTr("已停止"))
            color: root.simulationRunning ? "#35d19b" : "#d9e4ec"
            font.pixelSize: 16
        }
        ...
        Label {
            text: qsTr("已应用模式: ") + (root.acquisitionMode === "burst" ? qsTr("突发采集") : qsTr("连续采集"))
            color: "#d9e4ec"
            font.pixelSize: 16
        }

        Label {
            text: qsTr("已启用板卡 / 采集通道: ") + root.enabledBoardCount + qsTr(" / ") + root.enabledChannelCount
            color: "#d9e4ec"
            font.pixelSize: 16
        }
