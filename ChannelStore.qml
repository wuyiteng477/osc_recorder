...
QtObject {
    ...
    function historyValue(channelIndex, bufferIndex) {
        return channelBuffers[channelIndex] ? channelBuffers[channelIndex][bufferIndex] : undefined
    }
    ...
    function firstLogicalIndexAtOrAfter(time) {
        let low = 0
        let high = historyCount

        while (low < high) {
            const mid = Math.floor((low + high) / 2)
            const buffer = (historyStartIndex + mid) % historyCapacity

            if (historyTimes[buffer] < time)
                low = mid + 1
            else
                high = mid
        }
    ...
    function zeroCrossingFrequency(channelIndex, endTime, durationSeconds) {
        const first = firstLogicalIndexAtOrAfter(endTime - durationSeconds)
        const last = lastLogicalIndexAtOrBefore(endTime)
        ...
        for (let logical = first; logical <= last; ++logical) {
            const buffer = (historyStartIndex + logical) % historyCapacity
            const value = historyValue(channelIndex, buffer)
            ...
            if (!haveTime) {
                firstTime = historyTimes[buffer]
                haveTime = true
            }

            lastTime = historyTimes[buffer]
            previous = value
        }
    ...
    function updateFrame(channelIndex) {
        return updateFrames[channelIndex] || []
    }
    ...
    function initializeHistory() {
        if (historyTimes.length !== historyCapacity)
            historyTimes = new Array(historyCapacity)

        if (channelBuffers.length !== channelCount) {
            const buffers = []

            for (let i = 0; i < channelCount; ++i)
                buffers.push(new Array(historyCapacity))
    ...
    function appendSamples(startTime, interval, count) {
        ...
        for (let sample = 0; sample < count; ++sample) {
            ...
            for (let index = 0; index < channelModel.count; ++index) {
                ...
                if (data.enabled)
                    channelBuffers[index][writeIndex] = valueFor(data, time)
            }
    ...
    function buildUpdateFrames(latestTime, visibleSeconds, pointCount, channelIndices) {
        ...
        const limit = Math.min(channelIndices.length, maximumVisibleWaveforms)

        for (let item = 0; item < limit; ++item) {
            const index = channelIndices[item]
            const data = channel(index)
            ...
            for (let point = 0; point < pointCount; ++point) {
                const relative = pointCount > 1 ? point / (pointCount - 1) * visibleSeconds : 0
                frame[point] = valueFor(data, actualStart + relative)
            }
