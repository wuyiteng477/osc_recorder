...
RealtimeDataBackend::RealtimeDataBackend(QObject *parent) : QObject(parent)
{
...
float RealtimeDataBackend::valueAt(int channel, quint64 index) const { return isInHistory(index) ? m_raw[channel][index % Capacity] : std::numeric_limits<float>::quiet_NaN(); }
...
void RealtimeDataBackend::updateLevels(quint64 sampleIndex, int channelIndex, float value)
{
    ...
    for (auto &level : m_levels) {
        ...
        if (bucket.group != group) {
            ...
            bucket = { value, value, sampleIndex, sampleIndex, group };
        } else {
...
void RealtimeDataBackend::appendSimulatedSamples(double startTime, double sampleInterval, int count, const QVariantList &enabledChannels)
{
    ...
    for (const auto &item : enabledChannels) {
        ...
        if (index >= 0 && index < ChannelCount) m_enabled[index] = true;
    }
    for (int offset = 0; offset < count; ++offset) {
        ...
        for (int channel = 0; channel < ChannelCount; ++channel) {
            const float value = m_enabled[channel] ? valueFor(channel, time) : std::numeric_limits<float>::quiet_NaN();
...
QVariantList RealtimeDataBackend::envelopeSeries(int channel, quint64 first, quint64 last, quint64 groupSize, double start, double duration, int width) const
{
    ...
    for (quint64 group = firstGroup; group <= lastGroup; ++group) {
        ...
        if (bucket.group != group || !isInHistory(bucket.minIndex) || !isInHistory(bucket.maxIndex)) continue;
...
void RealtimeDataBackend::refreshDisplaySnapshot(double windowStart, double windowEnd, double sampleRate, int plotWidth, const QVariantList &visibleChannels)
{
    ...
    const double samplesPerPixel = sampleRate * duration / width;
    ...
    channels.reserve(visibleChannels.size());
...
{
    m_nextSample = 0; m_historyCount = 0; m_originTime = 0.0; m_displaySnapshot.clear();
    for (auto &channel : m_raw) std::fill(channel.begin(), channel.end(), std::numeric_limits<float>::quiet_NaN());
    for (auto &level : m_levels) std::fill(level.groups.begin(), level.groups.end(), std::numeric_limits<quint64>::max());
