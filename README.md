...
本轮只增强实时波形页：保留已有稳定显示，并增加真正由历史样本驱动的滚动显示、历史回看、清除历史和严格按用户操作写入的日志。其他四个占位页未修改。
...
稳定显示和滚动显示都会从同一个历史窗口绘制。稳定模式以每 20 ms（500 Hz 的整数周期）批量采样，使基波相位保持锁定而不横向爬动；缓慢的幅度、基线和事件仍会随时间体现出来。停止模拟只停止新样本写入，不影响回看。
...
运行后，先开始模拟并等待至少 20 秒以观察慢变化和脉冲事件；再分别在稳定、滚动模式中左移、右移和归零，检查顶部距离、底部时间范围以及实际波形窗口同步更新。随后停止模拟，确认历史仍可移动；执行垂直适配和位置复位，确认前者只改量程与垂直偏移，后者保留量程、时基和显示模式。
